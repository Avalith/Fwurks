...
// ================================================	
	...
	window.ib = new iBrowser;
	//-------------------------------------------------------------------------
	// open iBrowser
	function iBrowser_open() {			
		window.ib.oEditor.contentWindow.focus();
		var wArgs = {};
		var elm = window.ib.selectedElement;		
		if (elm != null) { // if element		
			if (elm.nodeName.toLowerCase() == 'img') { // selected object is image				
				...
				var oCaptionElement = window.ib.getNextSiblingByName(oImageElement, 'span');
				window.ib.getImgParam(oImageElement, wArgs); // set image parameters				)
				if (oLinkElement) { // set popup parameters					
					var oCaptionElement = window.ib.getNextSiblingByName(oLinkElement, 'span');
					window.ib.getLinkParam(oLinkElement, wArgs);
				}				
				...
				var oCaptionElement = window.ib.getNextSiblingByName(oLinkElement, 'span');
				window.ib.getLinkParam(oLinkElement, wArgs); // set popup parameters
				if (oImageElement) { // if first child is image					
					window.ib.getImgParam(oImageElement, wArgs); // set image parameters
				}
			} else if (elm.nodeName.toLowerCase() == 'span') { // selected element is caption container				
				var oCaptionElement = window.ib.getChildByName(elm, 'span');
				if (window.ib.isCaption(oCaptionElement)) { // check to see whether it is a caption element					
					if (elm.childNodes[0].nodeName.toLowerCase() == 'img') {
						var oImageElement = elm.childNodes[0];
						window.ib.getImgParam(oImageElement, wArgs);
			...
			window.ib.isCaption(oCaptionElement) ? wArgs.caption = 1 : '';
			window.ib.isCaption(oCaptionElement) ? wArgs.captionClass = oCaptionElement.parentNode.attributes['class'].value : '';
		}		
		//-------------------------------------------------------------------------
		// open iBrowser dialog		
		if (window.ib.isMSIE) { // IE
			var rArgs = showModalDialog(window.ib.baseURL, wArgs, 'dialogHeight:500px; dialogWidth:580px; scrollbars: no; menubar: no; toolbar: no; resizable: no; status: no;');  
			if (rArgs) { // returning from iBrowser (IE) and calling callback function				
				iBrowser_callback('','',rArgs);
			}
		} else if (window.ib.isGecko) { // Gecko 
			var wnd = window.open(window.ib.baseURL + '?editor=' + window.ib.editor + '&callback=iBrowser_callback', 'ibrowser', 'status=no, modal=yes, width=625, height=530');
		...
		if (rArgs) {
			if (!rArgs.action) { // no action set - image				
				if (!oImageElement) { // new image// no image - create new image
					window.ib.oEditor.contentWindow.document.execCommand('insertimage', false, rArgs.src);
					oImageElement = window.ib.getElementByAttributeValue(window.ib.oEditor.contentWindow.document, 'img', 'src', rArgs.src);		
				}
				
				// set image attributes
				window.ib.setAttrib(oImageElement, 'src', rArgs.src, true);				
				window.ib.setAttrib(oImageElement, 'alt', rArgs.alt, true);
				window.ib.setAttrib(oImageElement, 'title', rArgs.title, true);
				window.ib.setAttrib(oImageElement, 'align', rArgs.align, true);
				window.ib.setAttrib(oImageElement, 'border', rArgs.border);
				window.ib.setAttrib(oImageElement, 'hspace', rArgs.hspace);
				window.ib.setAttrib(oImageElement, 'vspace', rArgs.vspace);
				window.ib.setAttrib(oImageElement, 'width', rArgs.width);
				window.ib.setAttrib(oImageElement, 'height', rArgs.height);				
				window.ib.isMSIE ? window.ib.setAttrib(oImageElement, 'className', rArgs.className, true) : window.ib.setAttrib(oImageElement, 'class', rArgs.className, true);
				
				// set caption
				if (oImageElement.parentNode.nodeName.toLowerCase() == 'a') { // popup image
					var oLinkElement = oImageElement.parentNode;
					window.ib.setCaption(oLinkElement,rArgs.caption,oImageElement.getAttribute('title'),rArgs.captionClass);
				} else {
					window.ib.setCaption(oImageElement,rArgs.caption,oImageElement.getAttribute('title'),rArgs.captionClass);
				} 
			} else if (rArgs.action == 1) { // action set - image popup								
				if (oLinkElement) { // edit exiting popup link								
					a.href        = "javascript:void(0);";
					rArgs.popSrc  = escape(rArgs.popSrc);					
					window.ib.setAttrib(oLinkElement, 'title', rArgs.popTitle, true);
					window.ib.isMSIE ? window.ib.setAttrib(oLinkElement, 'className', rArgs.popClassName, true) : window.ib.setAttrib(oLinkElement, 'class', rArgs.popClassName, true);										
       				window.ib.setAttrib(oLinkElement, 'onclick', "window.open('" + rArgs.popUrl + "?url=" + rArgs.popSrc + '&clTxt=' + rArgs.popTxt + "','Image', 'width=500, height=300, scrollbars=no, toolbar=no, location=no, status=no, resizable=yes, screenX=100, screenY=100'); return false;", true);
				} else { // create new popup link		
					var oLinkElement = window.ib.oEditor.contentWindow.document.createElement('A');
					...
					if (window.ib.isMSIE) { // IE
						 if (elm.nodeName.toLowerCase() == 'span' || elm.nodeName.toLowerCase() == 'img') {								
							...
							var rng = window.ib.oEditor.contentWindow.document.selection.createRange();
							if (rng.text == '') {								
						...
						var sel = window.ib.oEditor.contentWindow.getSelection();						
						...
						window.ib.insertNodeAtSelection(window.ib.oEditor.contentWindow, oLinkElement);
					}
				}
			//-------------------------------------------------------------------------
			} else if (rArgs.action == 2) { // action set - delete popup link				
				window.ib.oEditor.contentWindow.document.execCommand('Unlink');
			}
		}
		return;
  	}	
	...
	iBrowser.prototype.setCaption = function (elm,chkCaption, caption, captionClass) {
		if (chkCaption == 1) { // set caption
			var doc = window.ib.oEditor.contentWindow.document;
			if (elm.nextSibling && elm.nextSibling.className == 'caption') { // existing caption
				...
				window.ib.isMSIE ? window.ib.setAttrib(capDiv, 'className', captionClass, true) : window.ib.setAttrib(capDiv, 'class', captionClass, true); 	// change class
				...
				if (window.ib.isMSIE) { // IE
					window.ib.setAttrib(capDiv, 'className', captionClass, true); 	// set class for caption container
					window.ib.setAttrib(capText, 'className', 'caption', true); 	// set class for caption text
					capDiv.innerHTML = elm.outerHTML;
					capDiv.appendChild(capText);  
					elm.outerHTML = capDiv.outerHTML;
				} else if (window.ib.isGecko) { // Gecko
					window.ib.setAttrib(capDiv, 'class', captionClass, true);		// set class for caption container
					window.ib.setAttrib(capText, 'class', 'caption', true);		// set class for caption text	
					var sel = window.ib.oEditor.contentWindow.getSelection();						
					capDiv.appendChild(capText);
					capDiv.insertBefore(elm,capDiv.firstChild);
					window.ib.insertNodeAtSelection(window.ib.oEditor.contentWindow, capDiv);
				}
			}
	...
	iBrowser.prototype.getSelectedElement = function () {		
		if (window.ib.isMSIE) {
			var sel = window.ib.oEditor.contentWindow.document.selection;
	...
	iBrowser.prototype.getElementByAttributeValue = function (node, element_name, attrib, value) {
		var elements = window.ib.getElementsByAttributeValue(node, element_name, attrib, value);
	...
	iBrowser.prototype.getElementsByAttributeValue = function (node, element_name, attrib, value) {
		...
		if (node.hasChildNodes) {
			for (var x=0, n=node.childNodes.length; x<n; x++) {
				var childElements = window.ib.getElementsByAttributeValue(node.childNodes[x], element_name, attrib, value);
