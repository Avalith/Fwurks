...
/*
class Smarty_Function_Scaffolding_Listing
{
...
function smarty_function_scaffolding_listing($params, $smarty)
{
	...
	$_pk 				= $_model::primary_keys(0);
	
	if($_user_can['add'])	{ $smarty->assign('listing_add', '<a class="button add" title="'.$_actions['add'].'" href="'.url_for('./add').'"><em class="icon add"></em><span>'.$_actions['add'].'</span></a>'); }
	...
	{
		$key = is_array($col_val) ? ( isset($col_val['col_class']) ? $col_val['col_class'] : $col_key ) : $col_val;
		...
		foreach ($_columns as $col_key => $col_val)
		{
			...
			if(is_array($col_val) && isset($col_val['function']))
			{
				foreach(explode('|', $col_val['function']) as $col_val_function)
				{
					...
					$val = $val_function($val, $key, $params['params'], $d, $_model, $_pk, (array)$col_val);
				}
			}
...
function smarty_function_scaffolding_listing_col__reserved_active($val, $key, $params, $data, $model, $pk, $col_params)
{
	isset($col_params['boolean_link']) || $col_params['boolean_link'] = true;
	return smarty_function_scaffolding_listing_col__boolean($val, $key, $params, $data, $model, $pk, $col_params);
}

function smarty_function_scaffolding_listing_col__reserved_title($val, $key, $params, $data, $model, $pk, $col_params)
{
	if(!$params['user_can']['edit'] || !Registry::$settings->edit_link_titles || ($params['check_editable'] && $data->_editable === '0')){ return $val; }
	
	return '<a href="'.url_for('edit/'.$data->$pk.'/').'">'.$val.'</a>';
}
