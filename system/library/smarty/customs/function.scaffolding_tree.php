<?php
/*
class Smarty_Function_Scaffolding_Tree
{
...
function smarty_function_scaffolding_tree($params, $smarty)
{
	...
	if($_user_can['add']){ $smarty->assign('tree_add', '<a class="button add" title="'.$_actions['add'].'" href="'.url_for('./add').'"><em class="icon add"></em><span>'.$_actions['add'].'</span></a>'); }
	if(!count($_data)){ $smarty->assign('tree', '<p class="no_entries">'.$_locales->ERRORS['no_entries'].'</p>'); return; }
	
	$last_level = $first_level = $_data[0]->level-1;
	...
	foreach($_data as $key => $d)
	{
		...
		if($_user_can)
		{
			$html .= '<input type="hidden" name="tree_order['.$d->primary.'][parent_id]" value="'.$d->parent.'" />';
			$html .= '<span class="items">';
			if($_user_can['edit'])		{ $html .= !$_check_editable || $_check_editable && $d->_editable === '1' 		? ' <a class="icon edit" title="'.$_actions['edit'].'" href="edit/'.$d->primary.'">'.$_actions['edit'].'</a>' : '<span class="icon edit-disabled"></span>'; }
			if($_user_can['delete'])	{ $html .= !$_check_deletable || $_check_deletable && $d->_deletable === '1' 	? ' <a class="icon delete" title="'.$_actions['delete'].'" href="delete/'.$d->primary.'">'.$_actions['delete'].'</a>' : '<span class="icon delete-disabled"></span>'; }
