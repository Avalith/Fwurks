<?php
/*
class Smarty_Function_Admin_Permissions
{
	public static function execute($params, $smarty, $template, $class='permissions')
	{
		$html = '<ul class="'.$class.'">';
		$params['struct'] || $params['struct'] = array();
		...
		foreach($params['struct'] as $k => $s)
		{
			...
			if(is_array($s['actions']) 	&& $s['type'] != 'hidden'){ $sub_a = self::execute(array('struct' => $s['actions']), $smarty, $template, ''); }
			if(is_array($s['menu'])		&& $s['type'] != 'hidden'){ $sub_m = self::execute(array('struct' => $s['menu']), $smarty, $template, ''); }
...
function smarty_function_admin_permissions($params, $smarty, $template, $class='permissions')
{
