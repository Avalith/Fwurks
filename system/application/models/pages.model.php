...
class Pages extends ActiveRecord 
{
	...
	{
		$attributes['slug'] = Inflector()->to_slug($attributes[ ($attributes['title'] && !$attributes['slug']) ? 'title' : 'slug' ]);
	}
	...
	protected function before_validation()
	{
		...
		else
		{
			if($this->old_storage->parent_id != $this->storage->parent_id)
			{
				...
				{
					$this->storage->parent_id = 0;
					$this->storage->nleft = self::$db->fetch(self::$db->query("SELECT MAX(nright) as max FROM {$this->table_name}"), true)->max + 1;
					$this->storage->nlevel = 1;
					$this->storage->nright = $this->storage->nleft + $this->old_storage->nright - $this->old_storage->nleft;
				}
			}
		}
	}
	...
	protected function before_validation_on_create()
	{
		...
		{
			$this->storage->parent_id = 0;
			$this->storage->nleft = self::$db->fetch(self::$db->query("SELECT MAX(nright) as max FROM {$this->table_name} WHERE navigation = 1" ), true)->max + 1;
			$this->storage->nlevel = 1;
	...
	protected function after_validation(){ unset($this->errors['slug']); }
	...
	protected function after_delete()
	{
		if($this->storage->id)
		{
			...
			self::$db->query("UPDATE {$this->table_name} SET nleft = nleft - {$summary} WHERE navigation = 1 AND nleft >= {$this->storage->nright}");
			self::$db->query("UPDATE {$this->table_name} SET nright = nright - {$summary} WHERE navigation = 1 AND nright >= {$this->storage->nright}");
		}
	}
}
