...
class AdminGroup extends ActiveRecord
{
	...
	protected function __relations()
	{
		return array
		(
			ORMRelation()->hasMany('AdminUsers')->through('AdminGroupsAdminUsers'),
		);
	}
	...
	protected function before_validation()
	{
		...
		isset($p['admin_users']) || $p['admin_users'] = array();
	...
	private function traverse_permissions($array)
	{
		...
		foreach($array as $k => $v)
		{
			...
			if(isset($v['menu'])){ $permissions = array_merge($permissions, $this->traverse_permissions($v['menu'])); }
		}
		return $permissions;
	}
}
